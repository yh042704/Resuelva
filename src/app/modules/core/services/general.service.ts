...
export class GeneralService<T> {
    ...
    Get(controller: string, action: string, queryParams?: HttpParams): Observable<T> {
    ...
    async GetFecth(controller: string, action: string, queryParams?: HttpParams) {
        let path = controller + "/" + (action != "" ? action : "");
    ...
    GetODATA(controller: string, queryParams?: HttpParams): Observable<T> {
    ...
    Post(controller: string, method: string, body: any, queryParams?: HttpParams, showSpinner: boolean = true): Observable<T> {
    ...
    /*Post2(controller: string, method: string, body: any, queryParams?: HttpParams, headers?: HttpHeaders): Observable<T> {
        let path = controller + "/" + (method != "" ? method + "/" : "");
        return this.http.post<IResponseWrapperDTO>(this.urlEndpoint + path, body, { params: queryParams, headers: headers }).pipe(
            map(response => ResposeHandler(response)),
            catchError(this.handleError)
        )
    }*/

    Put(controller: string, method: string, body: any, queryParams?: HttpParams, showSpinner: boolean = true): Observable<T> {
        // Obtener headers actualizados
        let httpHeaders = this.getHttpOptions();
        // Si el body es FormData, usa el conjunto de headers genérico, de lo contrario, usa httpHeaders
        httpHeaders = body instanceof FormData ? this.httOptionsDefaultGeneric : httpHeaders;
    ...
    Delete(controller: string, method: string, queryParams?: HttpParams, body?: any): Observable<T> {
        let path = controller + "/" + (method != "" ? method : "");
    ...
    loadFiles(controller: string, method: string, body: any, queryParams?: HttpParams): Observable<T> {
        let path = controller + "/" + (method != "" ? method + "/" : "");
...
class ODataDatasourceList {
    ...
    private service?: GeneralService<any> = undefined;

    constructor(service: GeneralService<any>, controller: string, method: string, select: any[], orderBy: string, customQuery: string, desc: boolean, customParameter: string, expand: any[] | undefined) {
